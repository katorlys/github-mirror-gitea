...
def check_gitea_org_exists(org_name):
    response = requests.get(f"{cache.HOST}/orgs/{org_name}", headers=cache.headers_json())
...
def create_gitea_org(org_name):
    ...
    response = requests.post(f"{cache.HOST}/orgs", headers=cache.headers_json(), data=json.dumps(data))
